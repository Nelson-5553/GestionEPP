                ...
                <div class="flex flex-col space-y-6">
                    ...
                    <div>
                        <div class="flex flex-row justify-start items-center space-x-2">
                            ...
                            <label for="direction" class="block text-gray-900 dark:text-gray-200 font-semibold">Sede</label>
                        </div>
                        <select id="sede_id" name="sede_id"
                        class="w-full appearance-none border text-gray-900 dark:text-gray-200 border-gray-300 dark:border-gray-700 bg-white dark:bg-neutral-700 rounded-md mb-3 focus:outline-none focus:ring-2 focus:ring-purple-500 mt-1">
                        @if ($sedes->isEmpty())
                            <option value="">No hay sedes disponibles</option>
                        @else
                            <option value="" disabled hidden>Selecciona una sede</option>
                            @foreach ($sedes as $sede)
                                <option value="{{ $sede->id }}" {{ $area->sede_id == $sede->id ? 'selected' : '' }}>
                                    {{ $sede->name }}
                                </option>
                            @endforeach
                        @endif
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3"
                            class="w-full mt-1 p-2 border rounded-lg bg-white dark:bg-neutral-700 text-gray-900 dark:text-gray-200" >{{ $area->description }}</textarea>
                    </div>
                </div>
