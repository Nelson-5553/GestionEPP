<div>
    @hasanyrole(['admin', 'supervisor'])
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
            ...
            <div wire:click="setEstado(null)"
                class="cursor-pointer flex flex-col justify-between aspect-video rounded-lg bg-blue-500 text-blue-800 dark:bg-blue-700 dark:text-blue-200 font-bold text-lg md:text-2xl p-4 lg:p-6 hover:scale-105 transition-transform ease-in-out duration-300">
                <div class="flex flex-row items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="hidden md:block size-6">
                ...
                <p id="countTotal" class="ml-1.5 text-md">{{ $totalSolicitudes }}</p>
            </div>

            <div wire:click="setEstado('Aprobado')"
                class="cursor-pointer flex flex-col justify-between aspect-video rounded-lg bg-green-500 text-green-800 dark:bg-green-700 dark:text-green-200 font-bold text-lg md:text-2xl p-4 lg:p-6 hover:scale-105 transition-transform ease-in-out duration-300">
                <div class="flex flex-row items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="hidden md:block size-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <p id="countAprobadas" class="ml-1.5 text-md">{{$aprobadas}}</p>
            </div>

            <div wire:click="setEstado('Rechazado')"
                class="cursor-pointer flex flex-col justify-between aspect-video rounded-lg bg-red-500 text-red-800 dark:bg-red-700 dark:text-red-200 font-bold text-lg md:text-2xl p-4 lg:p-6 hover:scale-105 transition-transform ease-in-out duration-300">
                <div class="flex flex-row items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="hidden md:block size-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <p id="countRechazadas" class="ml-1.5 text-md">{{$rechazadas}}</p>
            </div>

            <div wire:click="setEstado('Pendiente')"
                class="cursor-pointer flex flex-col justify-between aspect-video rounded-lg bg-yellow-500 text-yellow-800 dark:bg-yellow-700 dark:text-yellow-200 font-bold text-lg md:text-2xl p-4 lg:p-6 hover:scale-105 transition-transform ease-in-out duration-300">
                <div class="flex flex-row items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="hidden md:block size-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <p id="countPendientes" class="ml-1.5 text-md">{{$pendientes}}</p>
            </div>
        </div>
    @endhasanyrole
    ...
    @hasanyrole(['admin', 'supervisor'])
    <script>
        document.addEventListener('livewire:initialized', initCounters);
        document.addEventListener('livewire:update', initCounters);

        async function initCounters() {
            const { CountUp } = await import("https://cdn.jsdelivr.net/npm/countup.js@2.0.7/dist/countUp.min.js");

            const counterElements = ['countTotal', 'countAprobadas', 'countRechazadas', 'countPendientes'];

            counterElements.forEach(id => {
                const element = document.getElementById(id);
                if (element) {
                    const value = parseInt(element.innerText) || 0;
                    const countUp = new CountUp(id, value, { duration: 1 });
                    if (!countUp.error) {
